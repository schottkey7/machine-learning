...
class LearningAgent(Agent):
    ...
    def build_state(self):
        ...
        state = tuple([waypoint, inputs['light']])
    ...
    def get_maxQ(self, state):
        ...
        max_q = max([q for q in self.Q[state].values()])
        actions_with_max_q = [a for a, q in self.Q[state].items() if q == max_q]

        if len(actions_with_max_q) > 1:
            return choice(actions_with_max_q)

        return actions_with_max_q[0]
...
def run():
    ...
    agent = env.create_agent(LearningAgent, True, 0.85, 0.25)
    ...
    sim.run(tolerance=0.05, n_test=300)
